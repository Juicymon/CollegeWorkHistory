...
public class ShoppingListArrayList implements ShoppingListADT {
    ...
    public boolean remove(Grocery ent) {
        ...
        if (ent != null){ //checks to ensure remove call has value            
            //loop to search through each index in the arraylist
            for (int i = 0; i < shoppingList.size(); i++){               
                //compares the entered object with the object in the arraylist 
                //index
                if (shoppingList.get(i).compareTo(ent) == 0){ 
                    //if found, remove method is called, found is set to true
    ...
    public Grocery find(int index) throws IndexOutOfBoundsException,
        ...
        //if index is greater than the list indicies or less than 0, throw
