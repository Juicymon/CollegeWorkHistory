...
public class PyramidScheme extends LinkedTree<Person> {
    ...
    private ArrayList<MultiTreeNode<Person>> 
            findNodeChain(MultiTreeNode<Person> node, Person target) {
                ArrayList<MultiTreeNode<Person>> reverseList = new ArrayList<>();
                    ...
                    for(MultiTreeNode<Person> child : node.getChildren()){
                        try {
                            reverseList.addAll(findNodeChain(child, target));
                  ...
                  if(reverseList.size() > 0){
                   if (reverseList.get(0).getElement().equals(target)){
                       reverseList.add(node);
                   }
                   if(reverseList.contains(root)){
